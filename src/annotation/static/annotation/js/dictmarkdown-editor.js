...
const dictMarkdownHighlighting = styleTags({
  Text: tags.content,
  Bold: tags.strong,
  Italic: tags.emphasis,
  Ref: tags.link,
  Spaced: tags.monospace,
  Subscript: tags.compareOperator,
  Superscript: tags.arithmeticOperator,
});

// This file was generated by lezer-generator. You probably shouldn't edit it.
const parser = LRParser.deserialize({
  version: 14,
  states: "#lOQOPOOOVOPO'#C^ObOPOOOOOO'#Ch'#ChOOOO'#Cd'#CdOyOPO,58xOVOPO'#CaOVOPO'#CbOVOPO'#CcOOOO'#Ce'#CeQbOPOOOOOO-E6b-E6bOOOO1G.d1G.dO!XOPO,58{O!gOPO,58|O!uOPO,58}OOOO-E6c-E6cOOOO1G.g1G.gOOOO1G.h1G.hOOOO1G.i1G.i",
  stateData: "#T~OZPO~ORROSRO]RO~ORXOSXOZPO]XO^UO_VO`WO~ORROSROZ[O]RO~ORROSRO]RO^aO~ORROSRO]RO_bO~ORROSRO]RO`cO~O",
  goto: "!X]PP^PPaaaewPP}QQOTXQYQTPWZT]^_Q]UQ^VR_WQYQR`YaSPTUVW]^_",
  nodeNames: "⚠ DictMarkdown Bold Subscript Superscript Italic Ref Spaced",
  maxTerm: 16,
  propSources: [dictMarkdownHighlighting],
  skippedNodes: [0],
  repeatNodeCount: 2,
  tokenData: "$Q~R[Otwtu!iuzwz{!n{!bw!b!c!{!c#Qw#Q#R#Q#R#S#i#S;'Sw;'S;=`!c<%lOw~|V]~Otwuzw{!bw!c#Qw#S;'Sw;'S;=`!c<%lOw~!fP;=`<%lw~!nO`~~!sP^~z{!v~!{OZ~~#QO_~~#TR!Q![#^!c!}#^#T#o#^~#aP#Q#R#d~#iOS~~#lR!Q![#u!c!}#u#T#o#u~#xP#R#S#{~$QOR~",
  tokenizers: [0],
  topRules: {"DictMarkdown":[0,1]},
  tokenPrec: 0
});

const dictMarkdownSyntaxHighlighting = syntaxHighlighting(HighlightStyle.define([
    {
        tag: tags$1.content,
    },
    {
    ...
    {
        tag: tags$1.compareOperator,
        verticalAlign: "sub",
        fontSize: "small",
        color: "#006d91",
    },
    {
        tag: tags$1.arithmeticOperator,
        verticalAlign: "super",
        fontSize: "small",
        color: "#006d91",
    },
]));
const dictMarkdownLanguage = LRLanguage.define({
    name: "dictmarkdown",
...
AnnotationTypes.Bold = { name: "Bold", node: tags$1.strong, markup: "**" };
AnnotationTypes.Italic = { name: "Italic", node: tags$1.emphasis, markup: "*" };
AnnotationTypes.Reference = { name: "Ref", node: tags$1.link, markup: "@" };
AnnotationTypes.Spaced = { name: "Spaced", node: tags$1.monospace, markup: "$" };
AnnotationTypes.Subscript = { name: "Subscript", node: tags$1.compareOperator, markup: "_" };
AnnotationTypes.Superscript = { name: "Superscript", node: tags$1.arithmeticOperator, markup: "^" };
AnnotationTypes.All = [
...
CssClasses.PanelButton = "dictmarkdown-panel-button";
CssClasses.FormatButton = "dictmarkdown-formatt-button";
CssClasses.ActiveButton = "dictmarkdown-button-active";
CssClasses.EditorPanel = "dictmarkdown-editor-panel";
CssClasses.EmptyPanel = "dictmarkdown-empty-panel";
function getSyntaxNodeAtCursor(view) {
...
function createPanelButton(options, view) {
    ...
    icon.className = (_a = options.iconClassName) !== null && _a !== void 0 ? _a : "";
...
function toggleAnnotation(view, mark) {
    const { state } = view;
...
const makeBold = (view) => toggleAnnotation(view, AnnotationTypes.Bold.markup);
const makeItalic = (view) => toggleAnnotation(view, AnnotationTypes.Italic.markup);
const makeSuperscript = (view) => toggleAnnotation(view, AnnotationTypes.Superscript.markup);
const makeSubscript = (view) => toggleAnnotation(view, AnnotationTypes.Subscript.markup);
const makeSpaced = (view) => toggleAnnotation(view, AnnotationTypes.Spaced.markup);
const makeRef = (view) => toggleAnnotation(view, AnnotationTypes.Reference.markup);
...
function hanleEditorUpdate(update) {
    if (update.docChanged || update.selectionSet || update.focusChanged || update.viewportChanged) {
        ...
        for (const button of buttons) {
            const { annotationTypeName } = button.dataset;
            if ((node === null || node === void 0 ? void 0 : node.type.name) == annotationTypeName) {
...
const formattingUpdateListener = EditorView.updateListener.of(hanleEditorUpdate);
const editorPanelKeymap = keymap.of([
    { key: "Ctrl-Shift-b", run: (target) => { makeBold(target); return true; } },
    { key: "Ctrl-Shift-i", run: (target) => { makeItalic(target); return true; } },
    { key: "Ctrl-Shift-6", run: (target) => { makeSuperscript(target); return true; } },
    { key: "Ctrl-Shift--", run: (target) => { makeSubscript(target); return true; } },
    { key: "Ctrl-Shift-2", run: (target) => { makeRef(target); return true; } },
    { key: "Ctrl-Shift-4", run: (target) => { makeSpaced(target); return true; } },
    ...emacsStyleKeymap
]);
...
function resolveEditorParent(parent) {
...
function createDictmarkdownEditor(parent, options = {}) {
    const parentElem = resolveEditorParent(parent);
    let textChangeCallbacks = [];
...
function createDictmarkdownMergeEditor(parent, options) {
    var _a;
    const parentElem = resolveEditorParent(parent);
    let textChangeCallbacks = [];
    const view = new MergeView({
        ...
        b: {
            doc: (_a = options.initialText) !== null && _a !== void 0 ? _a : "",
            extensions: [
                minimalSetup,
                dictMarkdownLanguage,
                dictMarkdownSyntaxHighlighting,
...
export { createDictmarkdownEditor, createDictmarkdownMergeEditor };
//# sourceMappingURL=dictmarkdown-editor.esm.js.map
